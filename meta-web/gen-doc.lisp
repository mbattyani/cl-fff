...
(defun name-value-table (title list)
  (tt:table (:col-widths '(200 250) :border 0.1  :splittable-p t)
	    (tt::header-row ()
...
(defun modifiable-string (obj)
  (if (modifiable obj) "all"
      (format nil "~{~a~^, ~}"
	      (mapcar 'name (modifiable-groups obj)))))

(defmethod gen-doc-content ((fn function-info))
  (push (list (format nil "~a (function of ~a)" (string-downcase (name fn))
	...
	(content
	 (tt::compile-text ()
	    ...
	    (tt:paragraph (:font "Helvetica-Bold" :font-size 16 :top-margin 20)
			  "Function : " (tt::put-string (name fn)))
	    (tt:hrule :dy 2)
	    (tt::vspace 10)
	    (tt:paragraph (:font "Helvetica-Bold" :font-size 16 :top-margin 20)
         #+nil     (name-value-table (format nil "Caractéristiques générales de la fonction ~a" (name fn))
				  (append
				   (list
				    "Nom" (name fn)
		...
		(name-value-table (format nil "General properties of function ~a" (name fn))
				  (append
				   (list
				    "Name" (name fn)
				;    "French Name" (french (user-name fn))
				    "English Name" (english (user-name fn))
				    "Description" (description fn)
				    "Comment" (comment fn)
				    "Visible by" (visible-string fn)))))
...
(defmethod gen-doc-content ((slot slot-info))
  (push (list (format nil "~a (slot of ~a)" (string-downcase (name slot))
  ...
  (let* (*(tt::*pp-font-size* 12)
	(content
	 ...
	 (tt:paragraph (:font "Helvetica-Bold" :font-size 16 :top-margin 10)
             (name-value-table (format nil "General properties of slot ~a" (name slot))
	       (append
		(list
		 "Slot name" (name slot)
	;	 "French name" (french (user-name slot))
		 "English Name" (english (user-name slot))
		 "Name in SQL table" (meta::convert-name-to-sql-name (or (sql-name slot) (name slot)))
		 "Description" (description slot)
		 "Comment" (comment slot)
		 "Type" (meta::translated-choice-value 'value-type slot))
	       (when (eq (value-type slot) :object)
		 (list "Object type" (name (object-type slot))
		  ;     "French text when empty" (french (void-link-text slot))
		       "English text when empty" (english (void-link-text slot))
		       "Can create a new object" (yes-no (can-create-new-object slot))
		       "Create attribute object" (yes-no (create-new-object slot))
		       "Function to get object." (get-object-fn slot)
		       "Function to process new object" (process-new-object-fn slot)
		  ;     "French title for selection dialog" (french (get-value-title slot))
		       "English title for selection dialog" (english (get-value-title slot))
		  ;     "Texte FR pour dialog box de sélection" (french (get-value-text slot))
		       "English text for selection dialog" (english (get-value-text slot))
		       "HTML generation function" (get-value-html-fn slot)
		       ))
	       (when (eq (value-type slot) :other)
		 (list "Other type" (other-type slot)))
	       (list
		 "Stored in database" (yes-no (stored slot))
		 "In Proxy" (yes-no (in-proxy slot))
		 "Indexed" (yes-no (indexed slot))
		 "Unique" (yes-no (unique slot))
		 "Do not display null" (yes-no (dont-display-null-value slot))
		 "Default value" (initform slot)
		 "Unit" (unit slot)
		 "Visible by" (visible-string slot)
		 "Modifiable by" (modifiable-string slot)
		 "List of values" (yes-no (list-of-values slot))
		 )
	       (when (list-of-values slot)
		 (list "New objects on top" (yes-no (new-objects-first slot))))
	       (list
		 "View type" (meta::translated-choice-value 'value-type slot)
		 "HTML attributes" (html-tag-attributes slot)
		 "Duplicate value when copy" (yes-no (duplicate-value slot))
		 "Add \"Copy of\"" (yes-no (duplicate-value slot))
		 "Function to duplicate value" (duplicate-value-fn slot)
		 )
		)))
	 (when (choices slot)
	   (tt::vspace 20)
...
(defmethod gen-doc-content ((class class-info))
  ...
  (let* ((real-class (gethash class *class-info-to-class*))
	 (*class-doc* (format nil ", class ~a" (name class)))
	 (content
	 (tt::compile-text ()
          (tt:set-contextual-variable :classe (format nil " (class ~a)" (string-downcase (name class))))
	  ...
	  (tt::vspace 10)
	  (tt:paragraph (:font "Helvetica-Bold" :font-size 16 :top-margin 20)
		(name-value-table (format nil "General properties of class ~a" (name class))
			(list
		;	 "Nom français" (french (user-name class))
			 "English name" (english (user-name class))
			 "SQL table name" (meta::sql-name real-class)
			 "Description" (description class)
			 "Comment" (comment class)
			 "class GUID" (guid class)
			 "Version" (version class)
			 "Instanciable" (yes-no (instanciable class))
			 "In memory store" (yes-no (use-memory-store class))
			 "Super classes" (format nil "~{~a~^, ~}"
						      (mapcar 'name (direct-superclasses class)))
			 "Visible by" (visible-string class)
			 "Short Description" (if (short-description class)
						  (short-description class)
						  "see sources and super-classes")
			 )))
	  (tt::vspace 3)
	  :hfill (tt::graph-box (gen-class-graph-layout (project class) (list class) 1)) :hfill
	  (tt::vspace 10)
	  (tt::user-drawn-box :dx 400 :dy 16 :stroke-fn 'draw-class-graph-legend) :eol
	    ...
	    (tt::table (:col-widths '(100 120 30 170 30) :splittable-p t)
		     (tt::header-row ()
				     (tt::cell (:col-span 5 :background-color '(0.7 1.0 0.7))
					       (tt::paragraph (:font "Helvetica-Bold" :font-size 14)
							      "List of direct slots")))
		     (tt::header-row (:background-color '(0.7 1.0 0.7))
			 (tt::cell ()(tt::paragraph () "Name"))
			 (tt::cell ()(tt::paragraph () "Type"))
			 (tt::cell ()(tt::paragraph () "List"))
	      ...
	      (tt::table (:col-widths '(100 120 30 170 30) :splittable-p t)
		     (tt::header-row ()
			     (tt::cell (:col-span 5 :background-color '(1.0 0.7 0.7))
				       (tt::paragraph (:font "Helvetica-Bold" :font-size 14)
					 (tt::put-string (format nil "List of slots inherited from ~a"
								 (name super))))))
	  ...
	  (when (direct-functions class)
	    ...
	    (tt::table (:col-widths '(100 150 170 30) :splittable-p t)
		     ...
		     (tt::header-row (:background-color '(0.7 1.0 0.7))
			 (tt::cell ()(tt::paragraph () "Name"))
			 (tt::cell ()(tt::paragraph () "English name"))
			 (tt::cell ()(tt::paragraph () "Visible by"))
			 (tt::cell ()(tt::paragraph () "Page")))
		     (dolist (fonc (direct-functions class))
			   ...
			   (tt::cell ()(tt::paragraph (:font "Helvetica" :font-size 11)
						      (tt::put-string (english (user-name fonc)))))
			   (tt::cell ()(tt::paragraph (:font "Helvetica" :font-size 11)
	    ...
	    (when (direct-functions super)
	      ...
	      (tt::table (:col-widths '(100 150 170 30) :splittable-p t)
			 ...
			 (tt::cell (:col-span 4 :background-color '(1.0 0.7 0.7))
				   (tt::paragraph (:font "Helvetica-Bold" :font-size 14)
					(tt::put-string (format nil "List of functions inherited from ~a"
								(name super))))))
		     (tt::header-row (:background-color '(1.0 0.7 0.7))
			 (tt::cell ()(tt::paragraph () "Name"))
			 (tt::cell ()(tt::paragraph () "English name"))
	  ...
	  (tt:paragraph (:font "Helvetica-Bold" :font-size 14 :top-margin 20)
			"Description of the SQL tables for the class "
...
(defmethod gen-doc-content ((group class-group))
  (let ((content
	 (tt::compile-text ()
	  ...
	  (tt:paragraph (:font "Helvetica-Bold" :font-size 16 :top-margin 20)
			"Group of classes: " (tt::put-string (name group)))
	  (tt:hrule :dy 2)
	  (tt::vspace 10)
	  (tt:paragraph (:font "Helvetica-Bold" :font-size 16 :top-margin 20)
			(name-value-table "General properties"
			  (list
			   "Name" (name group)
			   "English Name" (english (user-name group))
			   "Description" (description group)
			   "Comment" (comment group)
			   "Number of classes" (length (classes group))
			   )))
	  (tt::vspace 10)
	  (tt::table (:col-widths '(100 315 35) :splittable-p t)
		 (tt::header-row ()
		     (tt::cell (:col-span 3 :background-color '(0.6 0.6 0.9))
			 (tt::paragraph () "List of classes")))
...
#+nil (push (list (format nil "~a (classe)" (name file)) file) *index*)
#+nil  (let ((content
	 (tt::compile-text ()
  ...
  (let ((content
	 (tt::compile-text
	  ...
	  (tt::paragraph (:h-align :center)
	     ...
	     (tt:hrule :dy 2)
	 #+nil    (tt::vspace 20)
	 #+nil    (tt::paragraph (:h-align :fill :font "Helvetica" :font-size 16 :color '(0.0 0 0.4)
	     ...
	     (tt::paragraph (:h-align :fill :font "Helvetica" :font-size 16 :color '(0.0 0 0.4)
				  :left-margin 25 :right-margin 25)
			"General Properties" (tt::dotted-hfill)
	     ...
	     (dolist (group (class-groups project))
	       (tt::paragraph (:h-align :fill :font "Helvetica" :font-size 14 :color '(0.0 0 0.4)
				    :left-margin 25 :right-margin 25)
			  (tt::format-string "Group of classes: ~a" (name group)) (tt::dotted-hfill)
		 ...
		 (tt::paragraph (:h-align :fill :font "Helvetica" :font-size 12 :color '(0.0 0 0.4)
			    ...
			    (tt::format-string "Class: ~a" (name class)) (tt::dotted-hfill)
			    (tt::format-string "~d" (tt::find-ref-point-page-number class)))
		 (tt::vspace 5)
		 (dolist (slot (direct-slots class))
		 ...
		 (dolist (fn (direct-functions class))
		   (tt::paragraph (:h-align :fill :font "Helvetica" :font-size 10 :color '(0.0 0 0.4)
					:left-margin 65 :right-margin 25)
			      (tt::format-string "Function: ~a" (name fn)) (tt::dotted-hfill)
	     ...
	     (tt:paragraph (:font "Helvetica-Bold" :font-size 16 :top-margin 20)
			   (name-value-table "General properties"
					     (list
					      ...
					      "Number of classes" (format nil "~d"
									(reduce '+ (class-groups project)
									  :key #'(lambda (g) (length (classes g)))))
					      "Number of users groups" (format nil "~d" (length (user-groups project)))
					      )))
	     (tt::vspace 20)
;********************
	     (tt::table (:col-widths '(100 315 35) :splittable-p t)
			(tt::header-row ()
		     (tt::cell (:col-span 3 :background-color '(0.6 0.6 0.9))
			 (tt::paragraph () "List of class groups")))
		 (tt::header-row (:background-color '(0.6 0.6 0.9))
		     (tt::cell ()(tt::paragraph () "Name"))
;		     (tt::cell ()(tt::paragraph () "Nom FR"))
		     (tt::cell ()(tt::paragraph () "Description"))
...
;********************
	  (tt::table (:col-widths '(100 350) :splittable-p t)
	     (tt::header-row ()
		     (tt::cell (:col-span 2 :background-color '(0.6 0.6 0.9))
		       (tt::paragraph () "List of users groups")))
	     (tt::header-row (:background-color '(0.6 0.6 0.9))
		     (tt::cell ()(tt::paragraph () "Name"))
		     (tt::cell ()(tt::paragraph () "Description")))
	  ...
	  (tt::vspace 20)
;********************
	  #+nil(tt::table (:col-widths '(100 315 35) :splittable-p t)
	     (tt::header-row ()
		     (tt::cell (:col-span 3 :background-color '(0.6 0.6 0.9))
...
(defun gen-doc (project)
  ...
  (let ((file (merge-pathnames (format nil "~a-app-doc.pdf" (name project)) (get-source-dir project)))
	(*margins* '(72 72 72 50))
	(pdf::*max-number-of-pages* 2000)
	(*index* nil)
	(*package* (ensure-package (project-package project)))
        (meta::*country-language* :en))
    (tt::with-document ()
