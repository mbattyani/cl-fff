...
(defmethod slot-value-using-class ((class fc-class) object slot-name)
  (let* ((slot (find slot-name (c2mop:class-slots class) :key 'c2mop:slot-definition-name))
    ...
    (if (eq (value-type slot) 'meta::decimal)
        (float (/ value (expt 10 (nb-decimals slot))) 1.0d0)
        value)))

(defmethod (setf slot-value-using-class) (value (class fc-class) object slot-name)
  (let ((slot (find slot-name (c2mop:class-slots class) :key 'c2mop:slot-definition-name)))
    (when (eq (value-type slot) 'meta::decimal)(setf value (fround (* value (expt 10 (nb-decimals slot))))))
    (if (in-proxy slot)
        (call-next-method)
        (let ((data (data-object object)))
          (unless data (setf data (load-object-data object)))
          (setf (slot-value data slot-name) value)))
    (when (eq (value-type slot) 'meta::decimal)(setf value (float (/ value (expt 10 (nb-decimals slot))) 1.0d0)))
    value))
...
(defun parse-date (string)
  (cond
    ((eql (mismatch string "ctime") 5)
     (+ #.(encode-universal-time 0 0 0 1 1 1970) (round (parse-integer string :start 5 :junk-allowed t) 1000)))
    ((eql (mismatch string "utime") 5)
     (parse-integer string :start 5 :junk-allowed t))
    (t (let* ((pos1 (position #\/ string))
...
(defun std-string-to-value (string slot)
  (let* ((*read-eval* nil)
    ...
    (if (choices slot)
        ...
        (cond
          ((eq type 'meta::date) (setf value (parse-date string)))
...
(defun default-value-to-sql-fn (type slot)
  (cond
    ...
    ((eq type 'meta::date) 'date-to-sql)
    ((eq type 'meta::universal-time) 'utime-to-sql)
    ((eq type 'meta::decimal) 'decimal-to-sql
     (let ((format (format nil "'~~,~df'" (nb-decimals slot))))
       (lambda (value stream)
         (decimal-to-sql value stream format))))
    ((eq type 'meta::ip-address) 'ip-to-sql)
...
(defun default-sql-to-value-fn (type slot)
  ...
  (cond
    ...
    ((eq type 'meta::date) 'parse-iso-date)
    ((eq type 'meta::universal-time) 'parse-iso-date)
    ((eq type 'meta::ip-address) 'parse-sql-ip)
...
(defun default-value-to-string-fn (type slot)
  ...
  (cond
    ((safe-subtypep type 'boolean) #'(lambda(value stream)
                                       (write-string (if value "true" "false") stream)))
    ((eq type 'meta::date) #'(lambda (value stream)
                         (if value
                             (utime-to-string-date value stream)
                             (write-string "null" stream))))
    ((eq type 'meta::universal-time) #'(lambda (value stream)
                                   (if value
                                       (utime-to-string-time value stream)
                                       (write-string "null" stream))))
...
(defun default-string-to-value-fn (type slot)
  (let ((choices (choices slot))
    ...
    (cond
      ...
      ((eq type 'meta::date) #'(lambda (string)
                           (string-to-date string null-allowed)))
      ((eq type 'meta::universal-time) #'(lambda (string)
                                     (string-to-utime string null-allowed)))
      ((eq type 'meta::decimal) #'(lambda (string)
                              (string-to-decimal string null-allowed)))
      ((eq type 'meta::ip-address) #'(lambda (string)
