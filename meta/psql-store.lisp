...
(defmethod save-modified-objects ((store psql-store))
  (with-store-lock (store)
   (when (modified-objects store)
     (with-psql-store-db (store)
       (let ((*forced-psql-connection* sql:*default-database*)
		...
		(if (list-of-values slot)
		    (let ((list nil))
			  ...
			  (sql-do-query
                           ((child-id)
                            (format nil
                              "SELECT id FROM ~a_~a WHERE parentid = ~d ORDER BY ordernb DESC"
			  ...
			  (sql-do-query
                           ((value)
                            (format nil
                               "SELECT value FROM ~a_~a WHERE parentid = ~d ORDER BY ordernb DESC"
                               (sql-name class)(sql-name slot)(id object)))
