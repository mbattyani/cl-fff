...
(defun verify-password (user password)
  (when user
    (string= (hash-password *app* *user* (login-name user) password) (password user))))

(defmethod login-name (user)
  )
...
(defmethod ensure-user (app)
  (unless *user*
    (switch-user (make-instance 'user :store meta::*memory-store*))))

(defun change-password-request-handler (request)
;  (unless *user*
      ...
      (html::html-to-stream
       ...
       ((:div :class "modal-header")
        ...
        ((:h4 :class "modal-title")
         (:translate '(:en "CHange password" :fr "Changer de mot de passe"))))
       ((:div :class "modal-body")
        (:jscript "function disable_register () {
   if ($('#passwordr').val() == $('#passwordr2').val()) {
       $('#register-btn').prop('disabled', false);
      ...
      $('#pswd-div').addClass('has-error');
        ...
        ((:form :class "form" :role "form" :method "post" :action (interface::url interface::*request*))
         ((:input :type "password" :class "form-control" :placeholder "Current password"
                  :name "password" :id "password"))
         ((:input :type "password" :class "form-control" :placeholder "New password"
                  :required "required" :id "passwordr" :name "password" :oninput "disable_register();"))
         ((:div :id "pswd-div")
          ((:input :type "password" :class "form-control" :placeholder "Confirm new password" :id "passwordr2"
                   :oninput "disable_register();")))
         ((:button :class "btn btn-lg btn-primary btn-block" :type "submit" :id "register-btn"
                   :name "register" :disabled "true") "Change password")))
       ((:div :class "modal-footer")
        ((:button :type "button" :class "btn btn-default" :data-dismiss "modal") "Close"))
        #+nil ((:div :class "modal-body")
...
;    (return-from change-password-request-handler))


(interface::add-named-url "/change-password.html" 'change-password-request-handler)
