...
(defparameter *dot-command* 
  #+unix (namestring (or (probe-file "/usr/local/bin/dot")
                         "/usr/bin/dot"))
  #-unix "\"c:\\software\\graphviz\\bin\\dot.exe\"")
...
(defgeneric adjust-graph-node-size (graph-node t real-1 real-2))
(defgeneric gen-graph-dot-data (graph t))
(defgeneric stroke-node-content (graph-node t))
	 ...
	 (progn
	   (with-open-file (s dot-file :direction :output :if-exists :supersede)
	     (gen-graph-dot-data graph s))
           (format t "~a~{ ~s~} ~s -o ~s"
                                     *dot-command* *dot-command-args* dot-file result-file)
           (print (list #+lispworks 'sys:call-system
                        #+cmu 'ext:run-program
                        #+sbcl 'sb-ext:run-program
                        (format nil "~a~{ ~s~} ~a -o ~a"
                                     *dot-command* *dot-command-args* dot-file result-file) :wait t))
           #+lispworks (sys:call-system (format nil "~a~{ ~s~} ~a -o ~a"
                                                *dot-command* *dot-command-args* dot-file result-file) :wait t)
           #+cmu (ext:run-program *dot-command*
                                  `(,@*dot-command-args* ,dot-file "-o" ,result-file) :wait t)
           #+sbcl (sb-ext:run-program *dot-command*
                                      `(,@*dot-command-args* ,dot-file "-o" ,result-file) :wait t)
           #+ccl(ccl:run-program  
                 *dot-command*
                 `(,@*dot-command-args* ,dot-file "-o" ,result-file))
           #+(not (or lispworks cmu sbcl ccl)) (error "No implementation provided run program on this implementation")
...
#+nil  (pdf:add-URI-link 0 0 100 100
                    "http://www.fractalconcept.com/asp/html/cl-pdf.html" :border #(1 0 0))
  )
